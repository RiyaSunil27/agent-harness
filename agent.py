"""
FAQ Agent — sliding window + token-threshold summarization + tool output offloading.

Context strategies applied in order each turn:
  1. Sliding window  — drop message pairs outside the last WINDOW_SIZE turns
  2. Summarization   — compress history to a summary when tokens exceed threshold
  3. Tool offloading — save old tool results to disk, replace with stubs (TODO)

Problem demonstrated: tool call results are JSON blobs that eat context fast.
Exercise: implement offload_tool_result() and compact_tool_messages() below.
"""
...
MODEL                 = "gpt-3.5-turbo"
MAX_TOKENS            = 512
CONTEXT_WINDOW        = 400             # artificially small to trigger compression quickly
...
def chat(client: OpenAI, system: str, history: list[dict], user_input: str) -> tuple[str, list[dict], bool]:
    ...
    while True:
        ...
        if msg.tool_calls:
            ...
            new_history = messages[1:]  # strip system prompt; includes user + tool exchanges + final reply
...
def main():
    ...
    print("=== FAQ Agent (Sliding Window + Summarization + Tool Offloading) ===")
